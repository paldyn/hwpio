...
### 포함
- **차트(Chart)**: SHAPE_COMPONENT + HWPTAG_CHART_DATA 구조 파싱
  - 차트 종류(막대/선/파이 등), 데이터 시리즈, 축 레이블, 제목 추출
  - SVG 렌더링 (네이티브 SVG path/text 생성)
- **OLE 개체(Ole)**: SHAPE_COMPONENT_OLE 구조 파싱
  - OLE 내부 프리뷰 이미지(PNG/BMP) 추출 → placeholder 렌더
  - 실제 OLE 내용 해석은 범위 외

### 제외
- 3D 차트, 복합 차트 고급 기능
- 차트 편집(읽기 전용)
- OLE 개체 실시간 편집/스트림 파싱
...
| 단계 | 내용 |
|------|------|
| 1 | CHART_DATA 레코드 스펙 조사 + IR 설계 (코드 변경 없음, 문서만) |
| 2 | Model 계층: ChartShape / OleShape 구조체 + ShapeObject enum 확장 |
| 3 | Parser 계층: shape_tag_id 분기 추가, CHART_DATA 파싱, OLE 프리뷰 이미지 추출 |
| 4 | Renderer 계층: SVG 출력 (1차는 막대/선/파이만) |
| 5 | 로컬 1.hwp(외부) 검증 + 자체 제작 차트 샘플 samples/에 추가 + 회귀 테스트 |
