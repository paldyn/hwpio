...
| 표 캡션(`hp:caption`) 미직렬화 | 캡션 subList 문단 소실 (ta-pic-001-r "&lt;그림 1&gt;…" 18자) — `diff_documents` 미비교라 baseline 통과 | 별도 이슈 후보 (#1379 4단계 발견) |
| secPr 페이지 여백 템플릿 하드코딩 | `hp:margin` left/right=8504 고정 방출 — 원본 여백 변형 (ta-pic-001-r 4252→8504) | 별도 이슈 후보 (#1166은 landscape/width/height만 동적화) |
