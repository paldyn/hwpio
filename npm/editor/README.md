...
> **[온라인 데모](https://hwpio.paldyn.com/)** 에서 먼저 체험해보세요.
...
| 옵션 | 기본값 | 설명 |
|------|--------|------|
| `studioUrl` | `https://hwpio.paldyn.com/` | rhwp-studio URL |
| `width` | `'100%'` | iframe 너비 |
...
> **주의:** rhwp-studio가 DEV 모드(vite dev server)로 빌드된 경우에만 `rhwpDev`가 등록됩니다. 프로덕션 빌드(기본 호스팅 URL `https://hwpio.paldyn.com/`)에는 포함되지 않습니다. 디버깅 도구가 필요하면 셀프 호스팅 환경에서 `vite dev` 모드로 실행하세요.
...
기본적으로 `https://hwpio.paldyn.com/`에 호스팅된 에디터를 사용합니다.
자체 서버에서 호스팅하려면:
