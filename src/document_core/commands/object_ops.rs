...
impl DocumentCore {
    ...
    ///
    /// cell_path_json: `[{"controlIdx":N,"cellIdx":M,"cellParaIdx":P}, ...]` 형식
    /// (마지막 segment 까지 셀 도달, inner_control_idx 가 셀 paragraph 내 그림 control idx).
    /// nested table 지원: path 배열 길이 늘리면 다단계 셀 진입.
    ...
    /// 캡션 신규 생성은 미지원 (caption_created 검출 시 NotSupported 에러).
    pub fn set_cell_picture_properties_by_path_native(
        ...
        if caption_created {
            return Err(HwpError::RenderError(
                "표 셀 내 그림에 캡션 신규 생성은 본 버전에서 지원하지 않습니다".to_string(),
            ));
        }
        let section = &mut self.document.sections[section_idx];
        section.raw_stream = None;
        self.recompose_section(section_idx);
        self.paginate_if_needed();
        // event log: outer table control idx 를 ctrl 로 기록
