...
fn diag_2024_vs_variant_line_segs() {
    ...
    eprintln!("variant total paragraphs: {}", doc_v.sections[0].paragraphs.len());
    eprintln!("2024    total paragraphs: {}", doc_24.sections[0].paragraphs.len());
    ...
    for i in 0..doc_v.sections[0].paragraphs.len().min(doc_24.sections[0].paragraphs.len()) {
        ...
        if !v_empty && !v24_empty {
            ...
            if v_first.line_height == v24_first.line_height && v_first.line_spacing == v24_first.line_spacing {
                both_filled_match += 1;
            } else {
                both_filled_diff += 1;
            }
    ...
    eprintln!("variant: line_segs={}, char_shapes={}", p_v.line_segs.len(), p_v.char_shapes.len());
    eprintln!("2024:    line_segs={}, char_shapes={}", p_24.line_segs.len(), p_24.char_shapes.len());
    if let Some(ls) = p_v.line_segs.first() {
        eprintln!("variant ls[0]: lh={} th={} bl={} ls={} cs={} sw={}",
            ls.line_height, ls.text_height, ls.baseline_distance, ls.line_spacing,
            ls.column_start, ls.segment_width);
    }
    for (i, ls) in p_24.line_segs.iter().enumerate() {
        eprintln!("2024    ls[{}]: vpos={} lh={} th={} bl={} ls={} cs={} sw={} tag=0x{:08X}",
            i, ls.vertical_pos, ls.line_height, ls.text_height, ls.baseline_distance,
            ls.line_spacing, ls.column_start, ls.segment_width, ls.tag);
    }
    // base_size 매칭
    let cs_2024_id = p_24.char_shapes.first().map(|cs| cs.char_shape_id).unwrap_or(0);
    let cs_v_id = p_v.char_shapes.first().map(|cs| cs.char_shape_id).unwrap_or(0);
    let base_2024 = doc_24.doc_info.char_shapes.get(cs_2024_id as usize).map(|cs| cs.base_size).unwrap_or(0);
    let base_v = doc_v.doc_info.char_shapes.get(cs_v_id as usize).map(|cs| cs.base_size).unwrap_or(0);
    ...
    if let (Some(v), Some(t)) = (ps_v, ps_24) {
        eprintln!("variant ParaShape: line_spacing={}, type={:?}, sp_before={}, ml={}, mr={}, indent={}",
            v.line_spacing, v.line_spacing_type, v.spacing_before, v.margin_left, v.margin_right, v.indent);
        eprintln!("2024    ParaShape: line_spacing={}, type={:?}, sp_before={}, ml={}, mr={}, indent={}",
            t.line_spacing, t.line_spacing_type, t.spacing_before, t.margin_left, t.margin_right, t.indent);
    }
    ...
    for i in 0..doc_v.sections[0].paragraphs.len() {
        ...
        if diff != 0 && i < 700 {
            ...
            eprintln!("p{}: chars={}, synth_lines={}, actual_lines={}, diff={:+}  {}",
                i, p_v.text.chars().count(), synth_count, actual_count, diff, text_pre);
        }
    }
        ...
        if !lh_match || !lc_match {
            ...
            if mismatch_count <= 15 {
                ...
                eprintln!(
                    ...
                    i, v0.line_height, v0.line_spacing, line_count_v,
                    t0.line_height, t0.line_spacing, line_count_24, text_pre
                );
            }
        }
    }
