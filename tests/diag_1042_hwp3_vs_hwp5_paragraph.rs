...
fn diag_paragraph_metrics_compare() {
    ...
    eprintln!("\n=== variant flag ===");
    eprintln!("HWP3 original: is_hwp3_variant={}", hwp3_doc.is_hwp3_variant);
    eprintln!("HWP5 variant : is_hwp3_variant={}", hwp5_doc.is_hwp3_variant);
    ...
    for &pi in &[199_usize, 200, 201] {
        ...
        if let (Some(h3p), Some(h5p)) = (hwp3_para, hwp5_para) {
            eprintln!(
                "HWP3 pi={} ps_id={} text='{}'",
                pi, h3p.para_shape_id,
                h3p.text.chars().take(40).collect::<String>()
            );
            eprintln!(
                "HWP5 pi={} ps_id={} text='{}'",
                pi, h5p.para_shape_id,
                h5p.text.chars().take(40).collect::<String>()
            );
            let h3_ps = hwp3_doc.doc_info.para_shapes.get(h3p.para_shape_id as usize);
            let h5_ps = hwp5_doc.doc_info.para_shapes.get(h5p.para_shape_id as usize);
            if let (Some(h3s), Some(h5s)) = (h3_ps, h5_ps) {
                eprintln!(
                    "HWP3 ParaShape: sb={} sa={} margin_l={} margin_r={} indent={} line_spacing={}",
                    h3s.spacing_before, h3s.spacing_after, h3s.margin_left,
                    h3s.margin_right, h3s.indent, h3s.line_spacing
                );
                eprintln!(
                    "HWP5 ParaShape: sb={} sa={} margin_l={} margin_r={} indent={} line_spacing={}",
                    h5s.spacing_before, h5s.spacing_after, h5s.margin_left,
                    h5s.margin_right, h5s.indent, h5s.line_spacing
                );
            }
            // line_segs 비교
