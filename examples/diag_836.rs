...
fn main() {
    ...
    let file = &args[0];
    let target_page: Option<u32> = args.windows(2)
    ...
    for sec_idx in 0..doc.document().sections.len() {
        ...
        for (pi, para) in section.paragraphs.iter().enumerate() {
            ...
            let ir_h: f64 = para.line_segs.iter()
                .map(|s| hwpunit_to_px(s.line_height + s.line_spacing, dpi))
                .sum();

            // Categorize
            let has_table = para.controls.iter().any(|c| matches!(c, Control::Table(_)));
            let has_shape = para.controls.iter().any(|c| matches!(c, Control::Shape(_) | Control::Picture(_) | Control::Equation(_)));
            let is_empty = para.text.trim().is_empty() && para.controls.is_empty();

            total_meas += mp.total_height;
            total_ir_progress += ir_h;

            if has_table { table_meas += mp.total_height; table_ir += ir_h; }
            else if is_empty { empty_meas += mp.total_height; empty_ir += ir_h; }
            else if has_shape { shape_meas += mp.total_height; shape_ir += ir_h; }
            else { text_meas += mp.total_height; text_ir += ir_h; }

            let diff = mp.total_height - ir_h;
            // 차이 큰 항목만 출력 (>5px diff)
            if diff.abs() > 5.0 || pi < 10 {
                let kind = if has_table { "TABLE" }
                    else if has_shape { "SHAPE" }
                    else if is_empty { "EMPTY" }
                    else { "TEXT" };
                let preview: String = para.text.chars().filter(|c| *c > '\u{1F}').take(30).collect();
                println!("  [{:4}] {} meas={:5.1} IR={:5.1} diff={:+6.1} segs={} ctrls={} '{}'",
                    pi, kind, mp.total_height, ir_h, diff,
                    para.line_segs.len(), para.controls.len(), preview);
            }
            let _ = prev_ir_vpos;
        }

        println!("\n--- SECTION {} TOTALS ---", sec_idx);
        println!("  TEXT  : meas={:.1}  IR={:.1}  diff={:+.1}", text_meas, text_ir, text_meas - text_ir);
        println!("  EMPTY : meas={:.1}  IR={:.1}  diff={:+.1}", empty_meas, empty_ir, empty_meas - empty_ir);
        println!("  SHAPE : meas={:.1}  IR={:.1}  diff={:+.1}", shape_meas, shape_ir, shape_meas - shape_ir);
        println!("  TABLE : meas={:.1}  IR={:.1}  diff={:+.1}", table_meas, table_ir, table_meas - table_ir);
        println!("  TOTAL : meas={:.1}  IR={:.1}  diff={:+.1}", total_meas, total_ir_progress, total_meas - total_ir_progress);
