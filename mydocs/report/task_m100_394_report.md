...
`rhwp-studio/src/engine/input-handler.ts` 단일 파일, 5 영역 주석 처리:
...
| 단계 | 작업 | 커밋 | 보고서 |
|------|------|------|--------|
| Stage 1 | 호출 지점 (line 1503, 1520) 주석 처리 | `d4f9ba6` | `mydocs/working/task_m100_394_stage1.md` |
...
향후 정책 복귀 시 다음 5 영역의 주석을 동시에 해제:

```bash
grep -n "Task #394" rhwp-studio/src/engine/input-handler.ts
```

5 영역 모두 마커가 있어 일괄 추적 / 정리 가능. 별도 task 로 깔끔히 정리 예정.
