...
4. **`--dpi <값>`** — `--scale` / `--max-dimension` / `--vlm-target` 모두 미지정 시 `scale = dpi / 96.0` 자동 계산 (#614)
5. (옵션 없음) — 기본 (scale 1.0, 페이지 native 크기)

여러 옵션 동시 지정 시 가장 작은 결과 선택 (안전 우선).

### DPI 메타데이터 (`--dpi`)

PNG `pHYs` chunk 영역 영역 DPI 메타데이터 영역 영역 인쇄 시점 크기 힌트 영역 영역 명시. **메타데이터 전용** — 실제 래스터 픽셀 수 영역 영역 영향 부재 (픽셀 수 영역 영역 `--scale` / `--max-dimension` / `--vlm-target` 영역 영역 결정).

- `--dpi 96` — 1.0× 동등 메타데이터 (기본 SVG 출력 영역 영역 정합)
- `--dpi 300` — 인쇄 표준 DPI (`--scale` 미지정 시 `scale=3.125` 자동)
- `--scale 2 --dpi 150` — `--scale` 명시 + DPI 메타데이터 만 별도 (auto-scale 미적용)

**`--dpi` 미지정 시** pHYs chunk 영역 영역 미삽입 (opt-in, 기본 PNG 동작 100% 보존).

`pHYs` chunk 영역 영역 PPM (pixels per meter) 영역 영역 변환: `ppm = round(dpi / 0.0254)`. PNG spec §11.3.5.3 정합.
...
# scale 명시 + DPI 메타데이터 만 별도 (auto-scale 미적용)
...
각 PNG 는:
- **포맷**: PNG (RGBA, 8-bit)
- **DPI**: `--dpi` 옵션 영역 영역 메타데이터 만 영향 (미지정 시 pHYs chunk 부재 — 디코더 영역 영역 96 DPI 가정)
...
- 후속 진전: PR #626 (P5 equation replay) → PR #720 (P6 raw SVG replay) → PR #734 (DPI 메타데이터, Task #614)
