...
| PR #1346 | WebCanvas layer adapter 축소 및 option metadata 분리 | 검토중 | PR 정보 확인, 로컬 검증, 리뷰 문서 작성 |
...
- [x] cherry-pick 후 검증:
  ...
  - `cd rhwp-studio && node --test tests/render-backend.test.ts` — 23 passed
  - `CARGO_INCREMENTAL=0 cargo check --lib --target wasm32-unknown-unknown -j 2` — 통과
- [ ] WASM 빌드 — Docker daemon 미실행으로 보류
  - 실패: `failed to connect to the docker API ... docker.sock ... no such file or directory`
